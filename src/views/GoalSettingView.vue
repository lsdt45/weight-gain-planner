...
<script setup lang="ts">
	...
	const formModel = reactive<FormModel>({
		targetWeight: userStore.userData.targetWeight || (userStore.userData.weight ? userStore.userData.weight + 1 : null) || null,
		timeframe: userStore.userData.timeframe || 8,
	});
	...
	const rules: FormRules = {
		targetWeight: [
			...
			{
				type: 'number',
				min: userStore.userData.weight ? userStore.userData.weight + 1 : 45,
				message: '目标体重必须大于当前体重',
				trigger: ['blur', 'change'],
			},
	...
	const calculateEstimatedWeeks = () => {
		if (formModel.targetWeight && userStore.userData.weight) {
			const weightToGain = formModel.targetWeight - userStore.userData.weight;
				...
				<n-space vertical size="large">
					...
					<n-form
						...
						<n-form-item label="目标体重 (kg)" path="targetWeight" required>
							<n-input-number
								v-model:value="formModel.targetWeight"
								:min="userStore.userData.weight ? userStore.userData.weight + 1 : 45"
								:max="150"
								placeholder="请输入目标体重"
								style="width: 100%"
						...
						<n-form-item label="期望时间 (周)" path="timeframe">
							<n-space vertical>
								<n-slider
									v-model:value="formModel.timeframe"
									:min="4"
									:max="24"
									:step="1"
									:marks="{
								...
								<n-space justify="space-between">
									...
									<n-text v-if="userStore.userData.weight && formModel.targetWeight">
										预计每周增重:
										{{ ((formModel.targetWeight - userStore.userData.weight) / formModel.timeframe).toFixed(2) }}kg
									</n-text>
								</n-space>
							</n-space>
						</n-form-item>
